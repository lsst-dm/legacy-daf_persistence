...
def dt_representer(dumper, data):
    """Represent an lsst.daf.base.DateTime (as ISO8601-formatted string in
    UTC)"""
    return dumper.represent_scalar(u'lsst.daf.base.DateTime', str(data))
...
def pl_representer(dumper, data):
    ...
    for name in data.getOrderedNames():
        pairList.append([name, data.get(name),
                data.getComment(name)])
...
def dt_constructor(loader, node):
    ...
    return lsst.daf.base.DateTime(str(dt))
