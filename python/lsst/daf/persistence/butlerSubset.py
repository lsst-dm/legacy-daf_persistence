...
class ButlerDataRef(object):
    ...
    def put(self, obj, datasetType=None, backup=False, **rest):
        """
        ...
        @param backup             if True, rename existing instead of overwriting
        @param **rest             keyword arguments with data identifiers

        WARNING: Setting backup=True is not safe for parallel processing, as it
        may be subject to race conditions.
        """

        if datasetType is None:
            datasetType = self.butlerSubset.datasetType
        self.butlerSubset.butler.put(obj, datasetType, self.dataId, backup=backup, **rest)
