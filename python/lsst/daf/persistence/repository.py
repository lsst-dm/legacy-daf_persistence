...
class Repository(object):
    ...
    * if parentJoin is 'outer' will return all results found.
    ...
    def __init__(self, cfg):
        ...
        # Only write a cfg to the repo if it does not already exist.
        if self.cfg['accessCfg.storageCfg.root'] is not None:
            if not self._access.exists('repoCfg.yaml'):
                self._access.write(ButlerLocation(pythonType=RepositoryCfg, cppType=None,
                                                  storageName='YamlStorage',
                                                  locationList=self._access.locationWithRoot('repoCfg.yaml'),
                                                  dataId={},
                                                  mapper=None), self.cfg)

        self._initMapper(self.cfg)
